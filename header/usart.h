...
void initUSART(void) {
	...
	UCSR0C = (1<<UMSEL00); // set to sync mode
	//UCSR0C = (0<<UPM00)|(0<<UPM01); //disable parity check
	//UCSR0C = (1<<UMSEL00) | (1<<UCSZ00) | (1<<UCSZ01);
}
...
void USART_Transmit(unsigned char data){		//For microcontroller 1
    /*Wait for empty transmit buffer*/
    while (!(UCSR0A & (1<<UDRE0)) ); //The UDRE Flag indicates if the transmit buffer (UDR) is ready to receive new data.
...
unsigned char USART_Receive(void) {
    ...
    while (!(UCSR0A & (1<<RXC0))); //This flag bit is set by the CPU when there are unread data in the Receive buffer and is cleared by the CPU when the receive buffer is empty.
